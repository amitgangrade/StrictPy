...
| **M48** | 993 | 22,674 | 26,971 | 15,415 | 0 | 0 | 0 | 16/0/0 | 13.1 |
| **M49** | **1016** | **22,716** | **28,074** | **15,645** | 0 | 0 | 0 | 16/0/0 | 13.1 |
| **M50a** | **1034** | **22,743** | **29,218** | **15,755** | 0 | 0 | 0 | 16/0/0 | 13.1 |
...
| M48 (`tabular` vs pandas 3.0 comprehensive benchmark suite — third benchmark-only milestone) | 16 | 0 | 0 | 13.1 ms | **NO Rust changes** — pure benchmark infrastructure (~2193 LOC of Python harness + CSV generators + .spy/.py pairs + JSON snapshot + Markdown report). Single agent, 4 separable per-phase commits (disjoint-handler — first commit at ~20%). Output: `bench/tabular_harness.py` + `bench/TABULAR_BENCH_REPORT.md` + `bench/history/m48_tabular.json`. **Headline findings (43 cells: 37 timed + 6 skipped)**: geomean 0.30× (StrictPy / pandas); 28W/1T/8L. StrictPy wins on all small cells (pandas import ~1s dominates) + read/filter/sort/rolling/describe/unique at medium+. Pandas wins decisively on hash-bucketing at medium+: group_by_str 11.2×, group_by+sum 11.25×, pivot_table 22.2×. **M49 target now numeric**: drive group_by_cat_via_strings at medium from 12.8s → <1.5s (~10× speedup). **Surprise**: pandas Categorical at 8 values gave 0.98× speedup — essentially no win. Codes-hash shines at high cardinality; M49 must add ~5000-value fixture. **Memory**: StrictPy peak RSS runs 4-5× pandas at large (M48b queued for memory deep-dive). **STOP CRITERIA cuts**: xl size + 8 large cells skipped (>30 min timeouts). Tests 993 → 993 (no Rust changes). Lesson 1 streak: 30 — **twelve consecutive `tabular`-related agents** (M37-M48). | — |
| M49 (`tabular` categorical codes optimization + polish — **MASSIVE bench-validated win**) | 16 | 0 | 0 | 13.1 ms | **PRIMARY WIN: ~70-194× speedup on categorical group_by depending on cardinality**. M48 predicted ~10×; actual result spectacularly exceeded prediction. M48 baseline group_by_cat_via_strings at medium = 12.8s; M49 = **66ms (~194×)**. At medium_card_5000 (high cardinality): 5.4s → **77ms (~70×)**. StrictPy now beats pandas's own Categorical fastpath by ~14× at high cardinality. **Most dramatic single-milestone perf win in the project since the M8 JIT cliff.** Single agent, 5 separable per-phase commits (disjoint-handler — first commit at ~20%), ~2700 LOC. Phase A: high-cardinality bench fixture + baseline (verification gate). Phase B: m38_groupby_* detects ColumnCategorical and hashes on codes[i] (i64) directly — transparent, no API change. Phase C: merge codes-hash (requires bit-identical categories[]); new constructors col_categorical_ordered + col_categorical_from_codes; new predicate cc.is_ordered(). Phase D: 1w/1M/1Y resample (calendar arithmetic + end-of-month clamping); outer-merge MultiIndex on either side (extends M46 fallback to all 3 cases); unstack distributes EVERY column (M46 only first); loc_range_multi_{i64,str,datetime}. Phase E: +23 tests + 1 demo (~230 LOC) + bench rerun + LANGUAGE_GUIDE §11.37/§11.38 new. NativeFn IDs 1061-1066 (6 new). **Bench-first methodology validated**: M48 + M49 establishes the template for fixing known perf gaps. Second bench-first cycle in the project (after M7-unfair → M7-fair → M8 in foundations). Edit-tool leak recurred ~10× but defensive cp worked. Lesson 1 streak: 31 — **thirteen consecutive `tabular`-related agents** (M37-M49). | — |
| M50a (`tabular.serve` HTTP transport + minimal browser-tab frontend — first M50 desktop UI milestone) | 16 | 0 | 0 | 13.1 ms | **The user's original Pandas-plan request finally implemented**: localhost HTTP server for interactive DataFrame exploration in a browser. Single agent, 2 commits (Phases A-D combined at ~50-70% of budget + Phase E), ~1580 LOC, **zero STOP CRITERIA cuts**. Architecture: hand-rolled HTTP/1.1 in `vm/src/builtins.rs::m50a_serve_loop` via `std::net::TcpListener` DIRECTLY — NOT via M28 socket stdlib or M29 framework. **No new crate deps** (no hyper/axum/tokio). ~850 LOC of Rust covers server + JSON serializers + 6 endpoints + bundled vanilla-DOM frontend. Surface: tabular.serve / serve_with_timeout. 5 endpoints: GET / (bundled HTML), GET /api/schema, GET /api/rows, GET /api/cell, POST /api/filter, POST /api/groupby. Server-side DataFrame ID registry (no GC integration needed). Bundled frontend: vanilla DOM ~200 LOC JS, lazy-scroll table + filter + groupby UI. **NEW cadence classification "net-new-feature"**: brief miscategorized as disjoint-handler; agent combined A-D because server + serializers + endpoints + frontend are tightly interlocked in builtins.rs. Four classifications now (M41-M50a octet). NativeFn IDs 1067-1068. M50b picks up frontend polish (sortable headers, composite filters, virtual scroll, CSV download, categorical serialization); M50c picks up pivot UI. Lesson 1 streak: 32 — **fourteen consecutive `tabular`-related agents** (M37-M50a). | — |
