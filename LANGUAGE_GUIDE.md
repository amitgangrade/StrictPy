...
### 12.8 Space Shooter — entity lists, AABB collision, parallax starfield (M57)

`examples/games/space_shooter.spy` is the third reference game: an 800×600 vertical shooter.  The player ship (a filled triangle drawn from `gfx.draw_line` scanlines) sits near the bottom and moves 8-directionally with the arrow keys; Space fires upward bullets (rate-limited to ~3/sec).  Enemy waves spawn at the top every ~1.1 s, fall, and randomly shoot back.  Three lives with a brief post-hit invulnerability flicker, a game-over overlay (R restarts, Esc quits), a live score, and a two-layer parallax starfield.  All art is `gfx` primitives — **no image/sprite assets**, which sidesteps the M53 image path and all sprite-licensing concerns.  Three WAV SFX (`shoot`/`explosion`/`hit`, plus an unwired `gameover`) are generated by the included `_generate_assets.py` (same square-wave/sweep helpers as Snake/Tetris).

Three patterns are new versus Snake/Tetris and worth knowing for any entity-driven game:

- **One `final class Entity` + `List[Entity]`, despawn-by-rebuild.**  Every moving thing (enemy / player-bullet / enemy-bullet / explosion) is an `Entity` with `kind: str`, float position/velocity (`x, y, vx, vy: f64`), `w, h: i32`, and `age_ms: i64`.  Positions are `f64` so sub-pixel velocity-×-dt motion is smooth; cast to `i32` only at draw time (`gfx.draw_rect(win, i32(e.x), i32(e.y), ...)`).  Spy lists have no in-place removal, so each frame's `despawn` and collision passes **rebuild a fresh `List[Entity]` from the survivors** — the same rebuild idiom Snake uses for its body and Tetris for the board.
- **AABB collision is just four float comparisons.**  `aabb_overlap(ax, ay, aw, ah, bx, by, bw, bh) -> bool` returns false on any separating axis (`ax + aw <= bx`, etc.), true otherwise.  Collision resolution uses a parallel `List[bool]` `dead` sized to the frame-start entity count `n`: pass 1 marks player-bullet/enemy pairs, pass 2 marks enemy/enemy-bullet vs the player, then one rebuild drops the dead and keeps everything (including explosions appended mid-pass at indices ≥ `n`).  O(n·m) per frame is trivially fast for the <50-of-each peak counts.
- **Parallax starfield from `List[Tuple[f64, f64]]`.**  Two layers of random points scroll down at different speeds (`y += speed * dt`), wrapping to the top with a fresh random `x` on exit.  Because tuples are immutable, `step_stars` **rebuilds the list** with updated coordinates rather than mutating in place, then `draw_stars` blits each with `gfx.draw_point`.  The far layer is dim and slow, the near layer bright and fast — cheap depth cueing.
